...
const SEMANTIC_CATEGORY_KEYWORDS: Record<string, string[]> = {
  CG_DEMOGRAPHICS:  ["age", "gender", "male", "female", "ethnicity", "language", "religion", "education"],
  CG_HOUSEHOLD:     ["household", "home office", "homebody", "family", "children", "parent", "spouse", "marital", "single", "renter", "homeown"],
  CG_FINANCE:       ["credit", "invest", "financial", "wealth", "net worth", "bank", "premium card", "card holder"],
  ...
  CG_OCCUPATION:    ["programmer", "analyst", "operator", "engineer", "developer", "occupation", "professional"],
  ...
  TXN_FITNESS:      ["fitness", "sport", "gym", "athletic", "supplement", "vitamin", "nutrition", "health"],
...
// ─── Individual Signal Size ───────────────────────────────────────────────────

function getIndividualSize(signal: Signal): number {
  // Age INT: derive from US Census range math
  if (
    ...
    return BASE_POPULATION * calculateAgeMultiplier(
      signal.data.range.min,
      signal.data.range.max
    );
  }

  // Field-name based sizing for known CG fields
  if (signal.type === "CONSUMER_GRAPH" && signal.data.field) {
    const field = signal.data.field;

    if (field === "gender")                  return BASE_POPULATION * 0.50;
    if (field.startsWith("hh_adults_"))      return BASE_POPULATION * 0.14;
    if (field.startsWith("hh_income_group")) return BASE_POPULATION * 0.10;
    if (field.startsWith("occupation"))      return BASE_POPULATION * 0.05;
    if (field.startsWith("interest_"))       return BASE_POPULATION * 0.20;
...
function getGroupKey(signal: Signal): string | null {
  ...
  // Group all age band signals together regardless of gender prefix
  if (
...
// ─── Field Signal Merging (OR logic for same-field signals) ──────────────────
...
function mergeFieldSignals(signals: Signal[]): MergedSignal[] {
  ...
  for (const [, group] of fieldGroups.entries()) {
    ...
    // Multiple signals for same field — OR logic
